"""Excel COM bridge for yield input, calculate, P&L read, and status cells."""
...
class ExcelBridgeError(RuntimeError):
    """Raised when Excel COM operations fail."""


def to_float(value: Any) -> float:
    """Convert Excel cell values to float (pure helper for tests)."""
    if value is None:
...
def is_calculation_done(state: Any) -> bool:
    try:
        return int(state) == XL_DONE
...
class ExcelBridge:
    def __init__(
        self,
        workbook_name: str = "sample.xlsx",
        sheet_name: str = "",
        input_cell: str = "D19",
        pnl_cell: str = "F22",
        status_cell: str = "J16",
        last_quote_cell: str = "J17",
        last_pnl_cell: str = "J18",
        last_action_cell: str = "J19",
        calculate_timeout_sec: float = 5.0,
    ) -> None:
        ...
        self.calculate_timeout_sec = calculate_timeout_sec
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ExcelBridgeError(
                "pywin32 is required for Excel COM. pip install pywin32"
            ) from exc
        ...
        try:
            ...
            raise ExcelBridgeError(
                "Failed to connect to running Excel.Application via GetActiveObject. "
                "Open sample/sample.xlsx in Excel and retry."
            ) from exc
    ...
    def _resolve_workbook(self) -> Any:
        ...
        if self.workbook_name:
            ...
            raise ExcelBridgeError(
                ...
                "Open sample/sample.xlsx and retry."
            )
    ...
    def _resolve_worksheet(self, wb: Any) -> Any:
        if self.sheet_name:
            try:
                ...
                raise ExcelBridgeError(
                    f"Worksheet '{self.sheet_name}' not found in workbook "
                    f"'{getattr(wb, 'Name', '?')}'."
                ) from exc
        ws = wb.ActiveSheet
        if ws is None:
    ...
    def calculate_and_wait(self, timeout_sec: Optional[float] = None) -> None:
        self._ensure()
        timeout = self.calculate_timeout_sec if timeout_sec is None else timeout_sec
        self._ws.Calculate()
        deadline = time.time() + max(0.1, timeout)
        while time.time() < deadline:
            try:
                state = self._app.CalculationState
            except Exception as exc:  # noqa: BLE001
                raise ExcelBridgeError(
                    f"Failed reading Excel CalculationState: {exc}"
                ) from exc
            if is_calculation_done(state):
                return
            time.sleep(0.05)
        raise ExcelBridgeError(
            f"Excel calculation timeout after {timeout}s "
            f"(CalculationState never became xlDone)."
        )
    ...
    def write_yield_calculate_read_pnl(self, yield_value: float) -> float:
        self.write_yield(yield_value)
        self.calculate_and_wait()
    ...
    def update_status(
        ...
        """Best-effort status cell updates; never raises to caller paths that must exit."""
        try:
            self._ensure()
