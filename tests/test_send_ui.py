...
from send.ui import SendError, activate_window, relative_point, send_text  # noqa: E402
...
def test_send_text_does_not_reforce_after_activate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("send.ui.pyperclip.paste", lambda: "hi")
...
def test_send_text_retries_focus_then_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    calls = {"activate": 0, "click": 0}

    def _activate(hwnd: int, cfg: object) -> None:
        calls["activate"] += 1

    def _click(*_a, **_k) -> None:
        calls["click"] += 1

    focus_ok = {"n": 0}

    def _same_app(a: int, b: int) -> bool:
        focus_ok["n"] += 1
        # first round: fg check fails (first _same_app False); second round ok
        if calls["activate"] == 1 and focus_ok["n"] == 1:
            return False
        return True
    ...
    class _Cfg:
        ...
        send_paste_pause_seconds = 0
        send_send_pause_seconds = 0
        send_foreground_retry_pause_seconds = 0

    monkeypatch.setattr("send.ui.ensure_target_window", lambda cfg: 1)
    monkeypatch.setattr("send.ui.activate_window", _activate)
    monkeypatch.setattr("send.ui._set_topmost", lambda hwnd, enabled: None)
    monkeypatch.setattr("send.ui._click_ratio", _click)
    monkeypatch.setattr("send.ui._same_app", _same_app)
    monkeypatch.setattr("send.ui.win32gui.GetForegroundWindow", lambda: 1)
    monkeypatch.setattr("send.ui.win32gui.WindowFromPoint", lambda pos: 1)
    monkeypatch.setattr("send.ui.win32gui.GetWindowText", lambda hwnd: "메모장")
    monkeypatch.setattr("send.ui.win32api.GetCursorPos", lambda: (0, 0))
    ...
    monkeypatch.setattr("send.ui.pyautogui.hotkey", lambda *a: None)
    monkeypatch.setattr("send.ui.pyautogui.press", lambda *a: None)
    monkeypatch.setattr("send.ui.time.sleep", lambda s: None)
    send_text("hi", _Cfg())
    assert calls["activate"] == 2


def test_send_text_clipboard_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    class _Cfg:
        send_process_name = "notepad.exe"
        send_window_title = "메모장"
        send_input_x = 0.5
    ...
    monkeypatch.setattr("send.ui.ensure_target_window", lambda cfg: 1)
    monkeypatch.setattr("send.ui.activate_window", lambda hwnd, cfg: None)
    monkeypatch.setattr("send.ui._set_topmost", lambda hwnd, enabled: None)
    monkeypatch.setattr("send.ui._click_ratio", lambda *a, **k: None)
    monkeypatch.setattr("send.ui._same_app", lambda a, b: True)
    monkeypatch.setattr("send.ui.win32gui.GetForegroundWindow", lambda: 1)
    ...
    monkeypatch.setattr("send.ui.win32api.GetCursorPos", lambda: (0, 0))
    monkeypatch.setattr("send.ui.pyperclip.copy", lambda text: None)
    monkeypatch.setattr("send.ui.pyperclip.paste", lambda: "other")
    with pytest.raises(SendError, match="clipboard mismatch after copy"):
        send_text("hi", _Cfg())
